...
@endif
			<section class="slider">
               ...
               $k=0;
               @endphp
               @foreach($dishSlider as $slider)
               <style>
         .w3l_banner_nav_right_banner<?php echo $k;?> { background:url("{{url('uploads/'.$slider->image)}}") no-repeat center;}        
               </style>
						<li>
							<div class="w3l_banner_nav_right_banner<?php echo $k;?>">
								<h3>{{$slider->title}}</h3>
								<div class="more">
								...
								<figure>
									<div class="snipcart-item block" >
										<div class="snipcart-thumb">
											<a href=""><img title=" " alt=" " src="{{url('uploads/'.$menu->image)}}" height="140" width="140" /></a>		
											<p>{{$menu->title}}</p>
											<h4>Rs. {{$menu->price}}</h4>
										</div>
										<div class="snipcart-details top_brand_home_details">
												...
												<fieldset>
													...
													<input type="hidden" name="item_name" value="{{$menu->title}}" />
													<input type="hidden" name="amount" value="{{$menu->price}}" />
													<input type="hidden" name="discount_amount" value="1.00" />
													<input type="hidden" name="currency_code" value="USD" />
													<input type="hidden" name="return" value=" " />
													<input type="hidden" name="cancel_return" value=" " />
													<input onclick="AddToCart('{{$product_id}}');" type="submit" name="submit" value="Add to cart" class="button" />
												</fieldset>
